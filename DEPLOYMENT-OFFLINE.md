...
5. **Offline install of the enterprise hardening features** — watchdog, Go scan accelerator, Postgres driver, IIS reverse proxy (Part 6)
6. **Offline update procedure** — how to apply future updates with no internet/git on the jump server (Part 7)

> **Note on Parts 1–5 below:** they still document the original NSSM-Windows-Service install
> path. `DEPLOYMENT.md` (the internet-connected guide) has since moved to running the web UI as
> a Scheduled Task under the gMSA via `Register-WebUIStartup.ps1` instead of NSSM — no new
> software download required, which is actually a better fit for an air-gapped/EDR-locked
> server than installing NSSM. If you're deploying fresh rather than maintaining an existing
> NSSM install, skip NSSM entirely: follow Parts 1–9 below for Python/gMSA/ports, then use
> `Register-WebUIStartup.ps1` (Part 6.3) instead of Part 4 step 10 / Part 5.4's `nssm install`
> steps. Existing NSSM installations don't need to be migrated on their own — leave them running
> as-is unless you're touching that specific piece anyway.
...
## Part 6 — Offline install of the enterprise hardening features

Each of the five hardening phases is optional (see `ENTERPRISE-HARDENING-RUNBOOK.md` for the
online version of these steps). Only Phases 2, 4, and 5 need anything downloaded — Phases 1 and 3
are pure code (already covered by the repo transfer in Part 1.5) and need nothing extra offline.

### 6.1 Phase 1 — Watchdog (no download needed)

`Watch-WebUIHealth.ps1` is plain PowerShell, already in the repo copy from Part 1.5. Register it
per `ENTERPRISE-HARDENING-RUNBOOK.md` Phase 1 once the web UI itself is running.

### 6.2 Phase 2 — Go network-scan accelerator

**On the internet-connected machine:**

| File | Source |
|---|---|
| `go1.23.x.windows-amd64.msi` | `https://go.dev/dl/` — pick the current stable release |

Copy the MSI to `C:\Transfer\` alongside everything else.

**On the jump server:**
```powershell
C:\Transfer\go1.23.x.windows-amd64.msi /quiet
# Open a new PowerShell window so PATH refreshes, then:
go version

cd C:\Apps\AD-Agent\tools\netscan
.\build.ps1
```

**If installing a Go toolchain on this server isn't acceptable per your change-control/EDR
policy**, build `netscan.exe` on a *different* Windows machine that has Go and is the same
architecture (amd64), then just copy the resulting `DCAnomalyAgent\bin\netscan.exe` binary to
the jump server — nothing about running the binary itself needs Go installed, only building it
does. This is often the better choice for a locked-down server: one fewer compiler present on a
production, EDR-protected box.

### 6.3 Phase 5 — IIS reverse proxy (authentication)

**On the internet-connected machine:**

| File | Source |
|---|---|
| `rewrite_amd64_en-US.msi` (URL Rewrite) | `https://www.iis.net/downloads/microsoft/url-rewrite` |
| `requestRouter_amd64.msi` (Application Request Routing 3.0) | `https://www.iis.net/downloads/microsoft/application-request-routing` |

Copy both to `C:\Transfer\`.

**On the jump server:**
```powershell
# IIS itself is a Windows Feature, not a download - no internet needed
Install-WindowsFeature -Name Web-Server, Web-Windows-Auth -IncludeManagementTools

# Install the two staged MSIs
Start-Process msiexec.exe -ArgumentList '/i C:\Transfer\rewrite_amd64_en-US.msi /quiet' -Wait
Start-Process msiexec.exe -ArgumentList '/i C:\Transfer\requestRouter_amd64.msi /quiet' -Wait

cd C:\Apps\AD-Agent\DCAnomalyAgent\Install
.\Register-IISReverseProxy.ps1 -AllowedGroup 'CONTOSO\AD-Agent-Analysts' `
    -LockWebUIToLocalhost -GmsaAccount 'CONTOSO\svc-discoverAgt$' `
    -PythonPath 'C:\Apps\Python312\python.exe'
```
See `ENTERPRISE-HARDENING-RUNBOOK.md` Phase 5 for `-CertThumbprint` guidance if you have a real
internal-CA certificate ready rather than using the self-signed one generated by default.

### 6.4 Phase 4 — PostgreSQL asset store (only if going past SQLite)

**On the internet-connected machine**, download the offline wheel the same way as Part 1.4:
```powershell
pip download psycopg2-binary --dest C:\AD-Agent-Wheels --platform win_amd64 --python-version 312 --only-binary=:all:
```
This adds one `.whl` to the same `AD-Agent-Wheels` folder you're already transferring — no
separate trip needed if you're re-running Part 1.4 anyway.

**On the jump server:**
```powershell
cd C:\Apps\AD-Agent\WebApp
.\.venv\Scripts\Activate.ps1   # or the embeddable python.exe directly, per Part 5.4
pip install --no-index --find-links C:\Transfer\AD-Agent-Wheels psycopg2-binary
```
The Postgres *server* itself is a separate provisioning question — if it's also air-gapped, use
your organization's normal offline Postgres install process; nothing about AD-Agent's side of
this is internet-dependent beyond the one wheel above.

---

## Part 7 — Offline update procedure

Mirrors `DEPLOYMENT.md` §12, adapted for a jump server with no internet and no `git`.

### 7.1 Before you update

- [ ] Record what's currently deployed — copy `C:\Apps\AD-Agent` to a dated backup location
      (`C:\Apps\AD-Agent-Backup-2026-08-15`) before touching anything. This is your rollback point,
      since there's no `git log` to fall back on without a repo.
- [ ] Confirm no scan/discovery is actively running (see DEPLOYMENT.md §12.1 for the command).
- [ ] Note current `/healthz` output as a baseline.

### 7.2 Bring the update across

On the internet-connected machine, zip the updated `AD-Agent` folder (or just the changed files,
if you're tracking that) and transfer it via your normal offline transfer method — same USB/file
share/software-distribution process as the initial install in Part 1.

### 7.3 Apply it

```powershell
# Replace changed files - safest is a full folder overwrite from the new zip,
# since there's no git diff to tell you exactly what changed
Expand-Archive C:\Transfer\AD-Agent-Update.zip -DestinationPath C:\Apps\AD-Agent -Force
```

Check whether `WebApp/requirements.txt` gained any new packages compared to your last install —
if so, transfer the additional wheel(s) per Part 1.4/6.4's pattern and install offline the same
way. Most updates won't add a new dependency.

### 7.4 Restart and verify

Same table as `DEPLOYMENT.md` §12.4 — if `WebApp/` changed, restart the web UI (Scheduled Task
or `nssm restart DCAnomalyWebUI`, whichever this server uses); PowerShell scripts/config need no
restart. Then verify against your Part 7.1 baseline:
```powershell
...
```powershell
# Stop whatever's running it first (Scheduled Task or nssm), then:
Remove-Item C:\Apps\AD-Agent -Recurse -Force
Rename-Item C:\Apps\AD-Agent-Backup-2026-08-15 C:\Apps\AD-Agent
# Restart the web UI task/service
```
This is why the dated backup copy in 7.1 matters more here than in the git-based flow — it's
the only rollback path without version history on the server itself.
